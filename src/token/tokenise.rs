...
use super::{binary::*, ident::*, literals::*, *};
...
#[inline]
fn skip_spaces(pos: &mut TokenPos, it: &mut Chars) {
    it.take_while_ref(|v| *v == ' ')
        .for_each(|_| pos.column += 1);
}

#[inline]
pub fn tokenise(str: String) -> (Vec<Token>, Vec<TokenisationError>) {
    let mut tokens = Vec::new();
    let mut errs = Vec::new();
    let mut it = str.trim().chars();
    ...
    loop {
        ...
        match c {
            ...
            'A'..='Z' | 'a'..='z' | '_' => tokens.push(tokenise_identifier(&mut pos, c, &mut it)),
